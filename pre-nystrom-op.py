...
def analyzeClusters(k, x, clus, shape):
	...
	for i in range(0, shape[0]):
		for j in range(0, shape[1]):
			if(c[i,j]) != bg:
				pa[c[i,j], 0] = pa[clus[i,j], 0] + 1 #Count
				pa[c[i,j], 1] = pa[clus[i,j], 1] + i #X-coordinate total
				pa[c[i,j], 2] = pa[clus[i,j], 2] + j #Y-coordinate total
				
	for i in range(0,k):
		if i!=bg and pa[i,0]!=0:
			pa[i,1] = pa[i,1]/pa[i,0]
			pa[i,2] = pa[i,2]/pa[i,0]
